...
export default function ContentDepthPanel({ result }: ContentDepthPanelProps) {
  ...
  return (
    <section
      className="border border-[var(--color-border)] bg-white p-5 mb-6"
    ...
    >
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5">
        {result.breakdown.map((check) => (
          ...
          >
            ...
            <span className={check.pass ? 'text-[var(--color-body)]' : 'text-[var(--color-primary)]'}>
              {check.message}
            </span>
          </li>
        ))}
      ...
      {failing > 0 && (
        <p className="mt-4 pt-3 border-t border-[var(--color-border)] font-mono text-[11px] text-[var(--color-muted)]">
