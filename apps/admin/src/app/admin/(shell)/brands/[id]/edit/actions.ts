...
// Mirror of updateProductSeo / updateCategorySeo. See those actions for
// rationale comments.
...
const UpdateBrandSeoSchema = z.object({
  id: z.string().uuid(),
...
export async function updateBrandSeo(formData: FormData): Promise<Result<void>> {
  try {
    const session = requireRole(await auth(), ROLES.CATALOGUE_WRITE)
    const parsed = UpdateBrandSeoSchema.parse({
      id: formData.get('id'),
    ...
    const before = await db.brand.findUnique({
      where: { id: parsed.id },
      select: {
        seoTitle: true,
        seoDescription: true,
    ...
    if (!before) return fail('NOT_FOUND', 'Brand not found')
    ...
    revalidatePath(`/admin/brands/${parsed.id}/edit`)
...
export async function uploadBrandOgImage(
  formData: FormData,
): Promise<
