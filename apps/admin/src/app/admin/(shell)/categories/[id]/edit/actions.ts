...
export async function updateCategorySeo(formData: FormData): Promise<Result<void>> {
  try {
    const session = requireRole(await auth(), ROLES.CATALOGUE_WRITE)
    const parsed = UpdateCategorySeoSchema.parse({
      id: formData.get('id'),
    ...
    if (!before) return fail('NOT_FOUND', 'Category not found')
    ...
    revalidatePath(`/admin/categories/${parsed.id}/edit`)
...
// ── uploadCategoryOgImage ──────────────────────────────────────────────────

const MAX_IMAGE_BYTES = 10 * 1024 * 1024
const ACCEPTED_IMAGE_TYPES = ['image/jpeg', 'image/png', 'image/webp', 'image/gif']

export async function uploadCategoryOgImage(
  formData: FormData,
): Promise<
