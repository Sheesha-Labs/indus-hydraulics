    ...
    <section className="border border-[var(--color-border)] bg-white">
      ...
      <form action={onSubmit} className="p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-[var(--color-border)]">
          ...
          {message && <span className="text-[12px] text-[oklch(0.4_0.14_145)] font-medium">{message}</span>}
          {error && <span className="text-[12px] text-[oklch(0.5_0.16_25)] font-medium">{error}</span>}
        </div>
      </form>
    </section>
  )
