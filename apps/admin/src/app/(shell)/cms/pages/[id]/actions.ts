...
// ── SEO tab — `updateCmsPageSeo` mirrors updateProductSeo ──────────────────
...
const UpdateCmsPageSeoSchema = z.object({
  id: z.string().uuid(),
...
export async function updateCmsPageSeo(formData: FormData): Promise<Result<void>> {
  try {
    const session = requireRole(await auth(), ROLES.CMS_WRITE)
    const parsed = UpdateCmsPageSeoSchema.parse({
      id: formData.get('id'),
    ...
    const before = await db.cmsPage.findUnique({
      where: { id: parsed.id },
      select: {
        seoTitle: true,
        seoDescription: true,
    ...
    if (!before) return fail('NOT_FOUND', 'CMS page not found')
    ...
    revalidatePath(`/cms/pages/${parsed.id}`)
...
export async function uploadCmsPageOgImage(
  formData: FormData,
): Promise<
