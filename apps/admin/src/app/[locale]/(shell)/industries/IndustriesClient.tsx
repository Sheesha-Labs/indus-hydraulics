...
export default function IndustriesClient({ locale, industries }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      ...
      {showCreate && <IndustryForm locale={locale} onDone={() => setShowCreate(false)} />}
              ...
              <div key={ind.id}>
                {isEditing ? (
                  <div className="border-t border-[var(--color-border)] bg-[var(--color-deep)] p-4">
                    <IndustryForm locale={locale} existing={ind} onDone={() => setEditingId(null)} />
                  </div>
                ) : (
    ...
    <>
      <button
        ...
        onClick={() => {
          ...
          setError(null)
          startTransition(async () => {
            const res = await deleteIndustry(id, locale)
