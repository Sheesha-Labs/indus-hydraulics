  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
          <Link href={`/${locale}/rfqs`} className="font-mono text-[12px] text-[var(--color-muted)] hover:text-[var(--color-primary)] mb-2 inline-block">
            ← RFQ Queue
          </Link>
        ...
        <div className="flex flex-wrap gap-2 mb-6 pb-6 border-b border-[var(--color-border)]">
          {validTransitions.map((transition) => (
            <form
              ...
              action={async () => {
                'use server'
                await updateRfqStatus(rfq.id, transition, locale)
              }}
            >
              <button
                type="submit"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="border border-[var(--color-border)] bg-[var(--color-elevated)] p-4">
            ...
            <Link href={`/${locale}/customers/${rfq.account.id}`} className="text-[13px] font-semibold text-[var(--color-accent)] hover:underline">
          ...
          <div className="border border-[var(--color-border)] bg-[var(--color-elevated)] p-4">
            ...
            <form action={async (fd: FormData) => {
              ...
              if (eId) await assignEngineer(rfq.id, eId, locale)
            }}>
