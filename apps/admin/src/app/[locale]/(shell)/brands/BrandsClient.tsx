...
export default function BrandsClient({ locale, brands }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      ...
      {showCreate && <BrandForm locale={locale} onDone={() => setShowCreate(false)} />}
              ...
              <div key={b.id}>
                {isEditing ? (
                  <div className="border-t border-[var(--color-border)] bg-[var(--color-deep)] p-4">
                    <BrandForm locale={locale} existing={b} onDone={() => setEditingId(null)} />
                  </div>
                ) : (
    ...
    <>
      <button
        ...
        onClick={() => {
          ...
          setError(null)
          startTransition(async () => {
            const res = await deleteBrand(id, locale)
