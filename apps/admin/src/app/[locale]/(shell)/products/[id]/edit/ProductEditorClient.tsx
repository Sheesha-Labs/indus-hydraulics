...
export default function ProductEditorClient({
  ...
  return (
    <div className="px-8 py-6 pb-16">
      ...
      {tab === 'core' && (
        <CoreTab locale={locale} product={product} brands={brands} categories={categories} onSaved={bumpSaved} />
      )}
      {tab === 'description' && <DescriptionTab locale={locale} product={product} onSaved={bumpSaved} />}
      {tab === 'commerce' && <CommerceTab locale={locale} product={product} onSaved={bumpSaved} />}
      {tab === 'specs' && (
        <SpecsTab
...
function TemplateSelector({
  ...
  function handleApply() {
    ...
    setError(null)
    startTransition(async () => {
      const res = await setProductSpecTemplate(productId, draftId || null, locale)
      if (!res.success) {
  ...
  return (
    <section className="bg-white border border-[var(--color-border)] p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        {currentTemplate && (
          <a
            href={`/${locale}/spec-templates/${currentTemplate.id}`}
    ...
    >
      ...
      {template.fields.length === 0 ? (
        <div className="px-5 py-6 text-[13px] text-[var(--color-muted)]">
          ...
          <a
            href={`/${locale}/spec-templates/${template.id}`}
      ...
      {specs.length > 0 &&
        Object.entries(groups).map(([group, items]) => (
          <div key={group} className="bg-white border border-[var(--color-border)]">
            ...
            {items.map((s) => (
              <SpecRow key={s.id} spec={s} productId={productId} locale={locale} onSaved={onSaved} />
            ))}
          </div>
        ))}

      <AddSpecForm productId={productId} locale={locale} onSaved={onSaved} />
    </section>
  )
}
...
function SpecRow({
  ...
  function onDelete() {
    ...
    startTransition(async () => {
      await deleteProductSpec(spec.id, productId, locale)
      onSaved()
    })
  }
...
function ImagesTab({
  ...
  function onDelete(id: string) {
    ...
    startTransition(async () => {
      await deleteProductImage(id, productId, locale)
      onSaved()
    })
  }
  ...
  function onReorder(id: string, dir: 'up' | 'down') {
    startTransition(async () => {
      await reorderProductImage(id, dir, productId, locale)
      onSaved()
    })
  }
...
function DocumentsTab({
  ...
  function onDelete(id: string) {
    ...
    startTransition(async () => {
      await deleteProductDocument(id, productId, locale)
      onSaved()
    })
  }
    ...
    <div className="flex flex-col gap-6 max-w-4xl">
      {crossRefs.length === 0 ? (
        ...
        <div className="bg-white border border-[var(--color-border)]">
          ...
          {crossRefs.map((c) => (
            <CrossRefRow key={c.id} cr={c} productId={productId} locale={locale} onSaved={onSaved} />
          ))}
        </div>
      )}
      <AddCrossRefForm productId={productId} locale={locale} onSaved={onSaved} />
    </div>
  )
}
...
function CrossRefRow({
  ...
  function onDelete() {
    ...
    startTransition(async () => {
      await deleteProductCrossReference(cr.id, productId, locale)
      onSaved()
    })
  }
...
function SeoTab({ locale, product, onSaved }: { locale: string; product: Product; onSaved: () => void }) {
  const [pending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(null)
