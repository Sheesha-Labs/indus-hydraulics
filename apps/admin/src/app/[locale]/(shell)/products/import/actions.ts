...
export async function commitProductImport(jobId: string, locale: string): Promise<Result<{ created: number; updated: number; failed: number }>> {
  try {
    ...
    revalidatePath(`/${locale}/products/import`)
...
export async function cancelImportPreview(jobId: string, locale: string): Promise<Result<void>> {
  try {
    requireRole(await auth(), ROLES.CATALOGUE_WRITE)
    z.string().uuid().parse(jobId)
