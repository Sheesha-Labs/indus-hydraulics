...
export default function SpecTemplatesClient({ locale, templates }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl">
      ...
      {showCreate && <CreateTemplateForm locale={locale} onDone={() => setShowCreate(false)} />}
        ...
        <div className="bg-white border border-[var(--color-border)]">
          ...
          {templates.map((t) => (
            ...
            >
              <div>
                <Link
                  href={`/${locale}/spec-templates/${t.id}`}
              ...
              <div className="flex items-center justify-end gap-2">
                ...
                <DeleteTemplateButton id={t.id} locale={locale} hasProducts={t.productCount > 0} />
              </div>
            </div>
          ))}
        </div>
...
function CreateTemplateForm({ locale, onDone }: { locale: string; onDone: () => void }) {
    ...
    <>
      <button
        ...
        onClick={() => {
          ...
          setError(null)
          startTransition(async () => {
            const res = await deleteSpecTemplate(id, locale)
