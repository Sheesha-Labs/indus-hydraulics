    ...
    <>
      ...
      <div className="px-8 py-6 pb-16">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-4 mb-4">
          <Card>
            <CardHead
              ...
              action={
                <Link href={`/${locale}/products`} className="text-[13px] text-[var(--color-accent)] hover:underline">
                  View all →
                </Link>
              }
            />
                  ...
                  {recentProducts.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-4 py-8 text-center text-[var(--color-muted)]">
                        No products yet.{' '}
                        <Link href={`/${locale}/products/new`} className="text-[var(--color-accent)] hover:underline">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-3 mb-4">
          {productsWithoutDatasheets > 0 && (
            <Alert type="warn">
              ...
              <Link href={`/${locale}/products`} className="underline">Review →</Link>
            </Alert>
          )}
          {productsWithoutDatasheets === 0 && (
            <Alert type="ok">
          ...
          <Alert type="info">
            ...
            <Link href={`/${locale}/rfqs`} className="underline">Open queue →</Link>
          </Alert>
          <Alert type="ok">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
          {[
            { label: 'Add product', href: `/${locale}/products/new`, icon: '+' },
            { label: 'Bulk import', href: `/${locale}/products/import`, icon: '↑' },
            { label: 'RFQ queue', href: `/${locale}/rfqs`, icon: '▤' },
            { label: 'Customer accounts', href: `/${locale}/customers`, icon: '◎' },
          ].map((link) => (
            <Link
              key={link.href}
              href={link.href}
