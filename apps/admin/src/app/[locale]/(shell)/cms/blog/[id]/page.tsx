...
async function savePost(formData: FormData) {
  ...
  if (id === 'new') {
    ...
    redirect(`/${locale}/cms/blog/${post.id}`)
  } else {
...
export default async function BlogPostEditorPage({ params }: Props) {
  ...
  return (
    <div className="max-w-[860px]">
      <div className="mb-6">
        <Link href={`/${locale}/cms?tab=blog`} className="font-mono text-[12px] text-[var(--color-muted)] hover:text-[var(--color-primary)] mb-2 inline-block">
          ← CMS
        </Link>
      ...
      <form action={savePost} className="space-y-5">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          {post?.isPublished && (
            <Link href={`/en/blog/${post.slug}`} target="_blank" className="font-mono text-[12px] text-[var(--color-accent)] hover:underline ml-2">
              View post ↗
            </Link>
          )}
        </div>
