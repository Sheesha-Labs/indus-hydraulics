  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {tab === 'blog' && (
          <Link href={`/${locale}/cms/blog/new`} className="h-9 px-4 flex items-center bg-[var(--color-accent)] text-white font-mono text-[12px] hover:opacity-90">
            + New Post
          </Link>
        )}
        {tab === 'pages' && (
          <Link href={`/${locale}/cms/pages/new`} className="h-9 px-4 flex items-center bg-[var(--color-accent)] text-white font-mono text-[12px] hover:opacity-90">
            + New Page
          </Link>
        )}
      </div>
              ...
              {posts.map((post, i) => (
                <div key={post.id} className={`grid grid-cols-[1fr_120px_100px_100px_80px] px-4 py-3.5 items-center bg-[var(--color-elevated)] ${i > 0 ? 'border-t border-[var(--color-border)]' : ''}`}>
                  ...
                  <div className="flex justify-end">
                    <Link href={`/${locale}/cms/blog/${post.id}`} className="font-mono text-[11px] text-[var(--color-accent)] hover:underline">
                      Edit
                    </Link>
                  </div>
                </div>
            ...
            <>
              ...
              {pages.map((page, i) => (
                <div key={page.id} className={`grid grid-cols-[1fr_100px_100px_80px] px-4 py-3.5 items-center bg-[var(--color-elevated)] ${i > 0 ? 'border-t border-[var(--color-border)]' : ''}`}>
                  ...
                  <div className="flex justify-end">
                    <Link href={`/${locale}/cms/pages/${page.id}`} className="font-mono text-[11px] text-[var(--color-accent)] hover:underline">
                      Edit
                    </Link>
                  </div>
                </div>
