...
const ADMIN_SRC = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
...
/** Every .ts/.tsx under apps/admin/src, relative to it. */
function sourceFiles(): string[] {
  ...
  const walk = (dir: string) => {
    for (const entry of readdirSync(dir)) {
      ...
      else if (/\.tsx?$/.test(entry)) out.push(path.relative(ADMIN_SRC, full))
    }
  }
  walk(ADMIN_SRC)
  return out
...
function scan(pattern: RegExp): Hit[] {
  ...
  for (const rel of files) {
    const lines = readFileSync(path.join(ADMIN_SRC, rel), 'utf8').split('\n')
...
describe('admin paths carry the /admin prefix', () => {
  ...
  test('no protocol-relative revalidatePath survives', () => {
    ...
    const bad: string[] = []
    for (const rel of files) {
      readFileSync(path.join(ADMIN_SRC, rel), 'utf8')
        .split('\n')
        .forEach((line, i) => {
          if (isComment(line)) return
