...
import { marketsOrdered, type Market } from '@indus/domain'
import { LeadCapturePanel, buildMailtoHref, buildWhatsappHref } from '@indus/ui'
import { getStoreSettings } from '../../lib/store-settings'

/**
 * The market page as it was before the designed template landed.
 *
 * Still the layout for the 125 markets that have no record in `MARKET_PAGES`
 * yet. It is not a fallback in the apologetic sense — it is a complete, honest
 * page built from the same registry row and the same live category tree, and
 * it stays until a market's own copy is written and reviewed.
 *
 * Lifted verbatim out of the route when the two layouts split, minus its
 * JSON-LD block: the route now emits Breadcrumb and Service for both layouts
 * so the two cannot drift apart on structured data.
 */

export default async function MarketLandingLegacy({ market }: { market: Market }) {
