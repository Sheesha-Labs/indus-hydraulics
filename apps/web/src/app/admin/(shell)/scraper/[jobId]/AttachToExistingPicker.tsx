...
export default function AttachToExistingPicker({ selected, onSelect, disabled }: Props) {
  ...
  if (selected) {
    return (
      <div className="border border-ih-border bg-ih-surface-2 px-3 py-2.5 text-[13px]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="font-mono text-[12px] text-ih-ink truncate">{selected.sku}</div>
            <div className="text-[12px] text-ih-ink-2 truncate">{selected.title}</div>
          ...
          <button
            ...
            className="text-[11px] font-mono uppercase tracking-wider text-ih-muted hover:text-ih-ink disabled:opacity-50"
          >
            Change
          </button>
        </div>
      ...
      {query.trim().length >= 2 && (
        <div className="border border-t-0 border-ih-border bg-ih-bg max-h-[260px] overflow-y-auto">
          {pending && hits.length === 0 && (
            <p className="px-3 py-2 text-[12px] text-ih-muted">Searching…</p>
          )}
          {!pending && hits.length === 0 && (
            <p className="px-3 py-2 text-[12px] text-ih-muted">No matches</p>
          )}
          {hits.map((h) => (
            <button
              key={h.id}
              type="button"
              onClick={() => onSelect(h)}
              className="w-full text-left px-3 py-2 border-t border-ih-border hover:bg-ih-surface-2 focus:bg-ih-surface-2 focus:outline-none"
            >
              <div className="flex items-baseline gap-3">
                <span className="font-mono text-[12px] text-ih-ink flex-shrink-0">
                  {h.sku}
                </span>
