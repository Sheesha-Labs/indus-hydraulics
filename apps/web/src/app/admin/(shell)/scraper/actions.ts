...
export async function cancelJob(jobId: string): Promise<Result<void>> {
  try {
    ...
    revalidatePath('/admin/scraper')
    revalidatePath(`/admin/scraper/${jobId}`)
    return ok(undefined)
  } catch (err) {
    return failFromError(err)
  }
...
export async function setRowSelection(
  ...
  try {
    ...
    revalidatePath(`/admin/scraper/${row.jobId}`)
    return ok(undefined)
  } catch (err) {
    return failFromError(err)
  }
...
export async function ingestRow(scrapedProductId: string): Promise<Result<{ productId: string }>> {
  try {
    ...
    revalidatePath(`/admin/scraper/${result.jobId}`)
    revalidatePath('/admin/products')
    if (!result.isNewProduct) revalidatePath(`/admin/products/${result.productId}/edit`)
