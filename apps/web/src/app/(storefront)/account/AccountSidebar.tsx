...
export default function AccountSidebar({ userName, userEmail, counts }: Props) {
      ...
      <Link
        ...
        className={`flex justify-between items-center px-3 py-2 text-[13px] transition-colors ${
          active
            ...
            : 'text-ih-ink-2 hover:bg-ih-surface'
        }`}
      >
        {label}
        {showCount && (
          <span
            className={`font-mono text-[11px] ${
              active ? 'text-[oklch(0.7_0_0)]' : 'text-ih-muted-2'
            }`}
          >
            {count}
          </span>
  ...
  return (
    <aside className="sticky top-[130px]">
      {/* User card */}
      <div className="flex items-center gap-2.5 p-3.5 border border-ih-border bg-ih-surface mb-4">
        <div className="w-10 h-10 rounded-full bg-ih-accent text-white grid place-items-center font-semibold text-sm flex-shrink-0">
          {initials}
        </div>
        <div className="min-w-0">
          <div className="font-semibold text-[13px] truncate">{userName}</div>
          <div className="font-mono text-[10px] text-ih-muted truncate">{userEmail}</div>
        </div>
      </div>
      ...
      */}
      <p className="font-mono text-[10px] tracking-[0.14em] text-ih-muted-2 px-3 pb-1.5 uppercase">
        Procurement
      </p>
      ...
      <p className="font-mono text-[10px] tracking-[0.14em] text-ih-muted-2 px-3 pt-3.5 pb-1.5 uppercase">
        Account
      </p>
      ...
      <button
        ...
        className="w-full flex items-center px-3 py-2 text-[13px] text-ih-muted hover:bg-ih-surface transition-colors text-left mt-1"
      >
        Sign out
      </button>
    </aside>
