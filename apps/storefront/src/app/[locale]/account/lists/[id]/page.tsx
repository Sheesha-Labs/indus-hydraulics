...
export default async function SavedListDetailPage({ params }: Props) {
  ...
  await getTranslations({ locale, namespace: 'account' })
  ...
  return (
    <div>
      ...
      <nav className="flex items-center gap-3.5 font-mono text-[12px] text-[var(--color-muted)] mb-4">
        ...
        <Link href={`/${locale}/account/lists`} className="hover:text-[var(--color-primary)]">Saved lists</Link>
        <span className="opacity-40">/</span>
        ...
        <div className="flex gap-2">
          ...
          {activeItems.length > 0 && (
            <form action={async () => {
              ...
              await convertToRfq(list.id, locale)
            }}>
              <button
                type="submit"
                ...
                {undoItems.map((item) => (
                  <div key={item.id} className="flex items-center justify-between px-4 py-3 bg-[var(--color-deep)]">
                    ...
                    <form action={async () => {
                      ...
                      await restoreItem(item.id, list.id, locale)
                    }}>
