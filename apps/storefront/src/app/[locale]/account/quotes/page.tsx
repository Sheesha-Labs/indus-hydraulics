...
export default async function AccountQuotesPage({ params, searchParams }: Props) {
    ...
    <div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
        <div>
          <div className="font-mono text-[11px] tracking-[0.14em] text-[var(--color-muted)] uppercase">
            <Link href={`/${locale}/account`} className="hover:text-[var(--color-primary)]">Account</Link>
            {' / '}Quotes
          </div>
      ...
      <div className="flex gap-0 border-b border-[var(--color-border)] mb-5 overflow-x-auto">
        {STATUS_TABS.map((tab) => {
          ...
          return (
            <Link
              key={tab.key}
              href={tab.key ? `/${locale}/account/quotes?status=${tab.key}` : `/${locale}/account/quotes`}
      ...
      {rfqs.length === 0 ? (
        <div className="py-16 border border-dashed border-[var(--color-border)] text-center">
          ...
          <Link href={`/${locale}/c`} className="mt-4 inline-block font-mono text-[12px] text-[var(--color-accent)] hover:underline">
            Browse products →
          </Link>
        </div>
      ) : (
