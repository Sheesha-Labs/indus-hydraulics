...
export default async function ComparePage({ params, searchParams }: Props) {
  ...
  function removeUrl(sku: string) {
    ...
    return remaining.length > 0 ? `/${locale}/compare?skus=${remaining.join(',')}` : `/${locale}/compare`
  }
    ...
    <div className="max-w-[1400px] mx-auto px-8 py-6 pb-20">
      ...
      {sortedProducts.length === 0 ? (
        <div className="py-20 text-center border-t border-[var(--color-border)]">
          <p className="text-[24px] font-semibold mb-3">{t('empty')}</p>
          <p className="text-[var(--color-muted)] text-sm mb-6">{t('emptyDescription')}</p>
          <Link href={`/${locale}/c`} className="inline-flex h-10 px-6 items-center bg-[var(--color-accent)] text-white text-sm font-medium hover:opacity-90">
            Browse Products
          </Link>
        </div>
      ) : (
                ...
                return (
                  <div key={product!.id} className="border-l border-[var(--color-border)] bg-[var(--color-elevated)] p-5 relative">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Link href={`/${locale}/quote`} className="h-8 flex items-center justify-center bg-[var(--color-accent)] text-white text-[12px] font-medium hover:opacity-90 transition-opacity">
                        Request quote
                      </Link>
                      <Link href={`/${locale}/p/${product!.sku}`} className="h-8 flex items-center justify-center border border-[var(--color-border)] text-[12px] text-[var(--color-body)] hover:bg-[var(--color-deep)] transition-colors">
                        View details →
                      </Link>
                    </div>
                  </div>
                      ...
                      return (
                        ...
                        >
                          {spec
                            ...
                            : <span className="text-[var(--color-muted)]">{t('noValue')}</span>}
                        </div>
                      )
                    })}
                    {Array.from({ length: emptySlots }).map((_, i) => (
