...
export default async function QuoteSubmitPage({ params }: Props) {
  const { locale } = await params
  await getTranslations({ locale, namespace: 'rfq' })
  const session = await auth()

  if (!session?.user?.accountId) {
    redirect(`/${locale}/sign-in?next=/${locale}/quote/submit`)
  }
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-8 py-8 pb-20">
      ...
      <div className="flex items-center gap-2 font-mono text-[11px] tracking-[0.1em] text-[var(--color-muted)] mb-2">
        <Link href={`/${locale}/quote`} className="hover:text-[var(--color-primary)] uppercase">Quote</Link>
      ...
      <RfqSubmitForm locale={locale} addresses={mappedAddresses} />
    </div>
  )
}
