...
| `202609241441_content_updated_at.sql` | Adds `"contentUpdatedAt"` to `products`, `categories`, `brands` and `industries`, maintained by triggers and read as the sitemap's `<lastmod>`. `updatedAt` moves on any write, so 1,485 of 1,487 products claimed the same five-minute change on 2026-08-25; the other three tables had no date at all. A BEFORE UPDATE trigger dates a row when any column outside a bookkeeping denylist changes (score, stock, sitemap and robots flags, `search_tsv`); AFTER triggers on the product child tables and on brand/industry case studies date the parent; a product joining, leaving or being renamed on a shelf dates that category or brand. Products are backfilled from `updatedAt` (the date already published); the other three stay NULL until a real edit. Tested against Postgres 17 before applying. | 2026-09-24 (as `content_updated_at`) |
