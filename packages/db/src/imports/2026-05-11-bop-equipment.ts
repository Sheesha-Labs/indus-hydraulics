 ...
 *   Brands             — created 0, updated 2
 *   Categories         — created 1, updated 9   ← see below
 *   Spec templates     — created 0, updated 3
 *   Products           — created 0, updated 22
 *   Megamenu nav links — deleted 0, inserted 9   ← see below
 *
 * RE-RUNNING IS DESTRUCTIVE, for two reasons:
 *
 * 1. The one category it would "create" is `bop-services`. That category was
 *    DELIBERATELY DELETED on 2026-05-12 by
 *    `src/imports/2026-05-12-bop-services-migration/`, which converted the 13
 *    BOP service products into ServiceCase rows. Recreating it partially
 *    undoes that migration.
 *
 * 2. `deleted 0, inserted 9` on the megamenu. The nav helper only removes
 *    *placeholder* leaves; the live ones are real category links, so a re-run
 *    adds 9 duplicates rather than replacing anything.
 * ══════════════════════════════════════════════════════════════════════════
 */
/**
