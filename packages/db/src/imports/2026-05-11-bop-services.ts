 ...
 *   FAILS with: Product "IH-BOP-SVC-PRESSURE-TEST-API53-INDUS" references
 *   unknown categorySlug "bop-services"
 *
 * SUPERSEDED — this file can no longer run, and that is correct.
 *
 * On 2026-05-12, `src/imports/2026-05-12-bop-services-migration/` deliberately:
 *   - created 10 ServiceCase rows to replace these 13 products
 *   - DELETED all 13 IH-BOP-SVC-* products and their specs/FAQs/documents
 *   - DELETED the `bop-services` category
 *   - removed the Services sub-section from the BOP megamenu column
 *   - added 301 redirects from /p/bop-* to /services/* (in apps/web/next.config.ts)
 *
 * Those redirects are live and verified. Resurrecting these products would
 * make 12 permanent redirects point at pages that exist again, which is worse
 * than either state alone.
 *
 * Kept ONLY as the record of what the catalogue looked like before that
 * migration. Read it; do not execute it.
 * ══════════════════════════════════════════════════════════════════════════
 */
/**
