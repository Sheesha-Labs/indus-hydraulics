...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:24px 0;border:1px solid #e7e5e4;background-color:#fafaf9;">
  <tr>
    <td style="padding:16px 20px;border-bottom:1px solid #e7e5e4;">
      <div style="font-size:11px;letter-spacing:0.08em;color:#78716c;text-transform:uppercase;margin-bottom:4px;">Reference</div>
      <div style="font-family:Menlo,Consolas,monospace;font-size:15px;color:#1c1917;">${escapeHtml(props.rfqCode)}</div>
    </td>
  </tr>
  <tr>
    <td style="padding:16px 20px;">
      <div style="font-size:11px;letter-spacing:0.08em;color:#78716c;text-transform:uppercase;margin-bottom:4px;">Items requested</div>
      <div style="font-size:15px;color:#1c1917;">${props.lineCount} line item${props.lineCount === 1 ? '' : 's'}</div>
    </td>
  </tr>
</table>
...
<p style="margin:0 0 24px 0;">
  <a href="${escapeHtml(props.trackingUrl)}" style="display:inline-block;background-color:#0c4a6e;color:#ffffff;text-decoration:none;padding:12px 24px;font-size:14px;font-weight:500;letter-spacing:0.02em;">View request status</a>
</p>

<p style="margin:0 0 8px 0;font-size:13px;color:#57534e;">If you need to add information or have urgent questions, just reply to this email — it goes straight to our sales desk.</p>
`.trim()
